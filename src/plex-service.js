...
class PlexService {
  ...
  async initialize(serverUrl, token) {
    ...
    this.thresholdSettings = {
      type: this.getFromStorage('unratedThreshold', 'percentage'),
      value: parseInt(this.getFromStorage('unratedThresholdValue', '50'), 10)
    };
  ...
  async fetchRandomUnratedAlbum(libraryId) {
    try {
      ...
      for (const album of shuffledAlbums) {
        ...
        const thresholdType = this.thresholdSettings.type;
        const thresholdValue = this.thresholdSettings.value;
      ...
      console.error('Error fetching random unrated album:', error);
      throw error;
    }
  }
